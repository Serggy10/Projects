...
void ComederoWiFi::loop()
{
    webSocketComedero.loop();
...
void ComederoWiFi::IniciaWiFi()
{
    ...
    // Iniciar el servidor webSocketComedero y asignar el callback
    webSocketComedero.begin();
    webSocketComedero.onEvent([this](uint8_t client_num, WStype_t type, uint8_t *payload, size_t length)
...
bool ComederoWiFi::IniciaWifiStation(String ssid, String password)
{
    ...
    Serial.println("STA direccion IP: " + String(WiFi.localIP().toString()));
    IPSTA = WiFi.localIP();
    return true;
}
...
void ComederoWiFi::onWebSocketEvent(uint8_t client_num, WStype_t type, uint8_t *payload, size_t length)
{
    ...
    {
        IPAddress ip = webSocketComedero.remoteIP(client_num);
        Serial.println("[" + String(client_num) + "] Connection from " + String(ip.toString()));
        ...
        webSocketComedero.sendTXT(client_num, jsonString);
        break;
    }
...
void ComederoWiFi::CargarParametros()
{
    ...
    Serial.printf("Numero de dosis: %d\n", numDosis);
    Serial.printf("SSID del router: %s\n", ssidRouter.c_str());
    Serial.printf("Contrasena del router: %s\n", passwordRouter.c_str());
...
{
    int contadorComederoAtascado = 0;
    rotarHorario = true;
    int pesoInicial = peso; // por si hay alguna deriva del peso con el tiempo que siempre saque la misma cantidad, aun que en la bascula haya más o haya menos peso.
                            // Serial.println("Peso inical:" + (String)pesoInicial);
    unsigned long int tiempoInicio = millis();
    while (peso < dosisSinricPro + pesoInicial)
    {
        CalculaPeso();
        MoverMotor();
        // Serial.println("rotarAntihorario: " + String(rotarAntihorario ? "true" : "fasle"));
        // Serial.println("rotarHorario: " + String(rotarHorario ? "true" : "fasle"));
        if (millis() > tiempoInicio + 10000) // 20seg
        {
            contadorComederoAtascado++;
            rotarHorario = !rotarHorario;
            rotarAntihorario = !rotarAntihorario;
            tiempoInicio = millis();
            // Serial.println(contadorComederoAtascado);
            if (contadorComederoAtascado >= 3)
            {
                // Serial.println("Comedero atascado");
                comederoAtascado = true;
                rotarHorario = false;
                rotarAntihorario = false;
                return;
            }
...
void ComederoWiFi::CalculaPeso()
{
    // Serial.println("valorMinBascula: " + String(valorMinBascula));
    // Serial.println("valorCalibBascula: " + String(valorCalibBascula));
    // Serial.println("PesoReferencia: " + String(pesoReferencia));
    peso = map(bascula.read_average(10), valorMinBascula, valorCalibBascula, 0, pesoReferencia);
...
void ComederoWiFi::MandarPeso()
{
    if (clienteConectado)
        if (millis() > tiempoActualizarPeso + 500)
        {
            ...
            webSocketComedero.broadcastTXT(jsonString);
        }
}
...
void ComederoWiFi::DosisSinricPro(int dosis)
{
    darDosisSinricPro = true;
    dosisSinricPro = dosis;
}

void ComederoWiFi::DosificarSinricPro()
{
    if (darDosisSinricPro && !(rotarHorario || rotarAntihorario))
    {
        int contadorComederoAtascado = 0;
        rotarHorario = true;
        int pesoInicial = peso; // por si hay alguna deriva del peso con el tiempo que siempre saque la misma cantidad, aun que en la bascula haya más o haya menos peso.
        ...
        while (peso < dosisSinricPro + pesoInicial)
        {
            ...
            // Serial.println("rotarAntihorario: " + String(rotarAntihorario ? "true" : "fasle"));
            // Serial.println("rotarHorario: " + String(rotarHorario ? "true" : "fasle"));
            if (millis() > tiempoInicio + 10000) // 10seg
            {
                contadorComederoAtascado++;
                rotarHorario = !rotarHorario;
                rotarAntihorario = !rotarAntihorario;
...
void ComederoWiFi::ComprobarConexion()
{
    if (WiFi.status() != WL_CONNECTED && conectadoWifi)
    {
        Serial.println("Perdida de conexion");
        conectadoWifi = false;
    }

    if (millis() > tiempoReestablecerConexion + 60000 && !conectadoWifi)
    {
        tiempoReestablecerConexion = millis();
        Serial.println("intentando reconectar");
        conectadoWifi = IniciaWifiStation(ssidRouter, passwordRouter); // Intentar reconectar
    }
}
